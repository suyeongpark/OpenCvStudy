#include "pch.h"
#include "Study10kNN_Test.h"


Study10kNN_Test::Study10kNN_Test()
{
}
...
void Study10kNN_Test::make_trainData(Mat trainData, Mat group[2], Mat & classLable)
{
...
void Study10kNN_Test::draw_points(Mat & image, Mat group[2])
{
...
void Study10kNN_Test::kNN_test(Ptr<ml::KNearest> knn, int K, Mat& image)
{
	for (int y = 0; y < image.rows; y++)
	{
		for (int x = 0; x < image.cols; x++)
		{
			...
			if (resp == 1)
			{
				image.at<Vec3b>(y, x) = Vec3b(0, 180, 0);
			}
			else
			{
				image.at<Vec3b>(y, x) = Vec3b(0, 0, 180);
			}
		}
	}
}
