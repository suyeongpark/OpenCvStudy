...
int Study9DCT_fast::Start()
{
	...
	printf("fast DFT + IDFT 시간 %5.1f ms\n", (double)(clock() - start));
...
Mat Study9DCT_fast::calc_cosine(int N, int M)
{
...
Mat Study9DCT_fast::DCT_block(Mat g, Mat cos_mat, int dir)
{
		...
		for (int l = 0; l < M; l++)
		{
			...
			for (int n = 0; n < N; n++)
			{
				for (int m = 0; m < M; m++)
				{
					...
					if (dir == 0)
					{
						dst.at<float>(k, l) += ck * cl * coef * cos_value;
					}
					else
					{
						dst.at<float>(n, m) += ck * cl * coef * cos_value;
					}
				}
			}
		}
...
Mat Study9DCT_fast::DCT_block(Mat g)
{
...
Mat Study9DCT_fast::IDCT_block(Mat f)
{
...
void Study9DCT_fast::DCT_2D_fast(Mat img, Mat & dst, int N, int M, int dir)
{
...
void Study9DCT_fast::DCT_2D(Mat img, Mat & dst, int N, int M, int dir)
{
	dst = Mat(img.size(), CV_32F);
	img.convertTo(img, CV_32F);
